...
const rules={owner:'2 — Déplacer une pièce adverse (sauf sa dame)',pawn:'4/6 — Pion : une case orthogonale',knight:'7 — Cavalier : déplacement en L',jump:'9 — Sauter par-dessus une pièce',king:'12 — Roi : une seule case',bishop:'13 — Fou : diagonales',rook:'14 — Tour : lignes et colonnes',queen:'18 — Dame : lignes, colonnes ou diagonales',friend:'23/24 — Capturer une pièce amie sans tour',outside:'25 — Sortir du plateau sans être un fou',mine:'26 — Déplacer un livre',sleep:'27 — Déplacer une pièce couchée',castle:'28 — Déplacer un château',affair:'29 — Dame capturant le roi adverse'};
...
function move(s,source,x,y,random=Math.random){
 ...
 if(source.mine&&!s.mines.includes(key(p.x,p.y)))throw Error('mines absente.');
 ...
 s.pending=null;
 const text=`${actor==='w'?'Blancs':'Noirs'} : ${names[p.type]||'mines'} ${label(p.x,p.y)} → ${label(x,y)}${authorized?' (vengeance)':''}`;
 if(p.type==='mine'){
  ...
  if(target)s.pieces=s.pieces.filter(q=>q.id!==target.id);
  p.x=x;p.y=y;
  if(s.mines.includes(key(x,y))){s.mines=s.mines.filter(k=>k!==key(x,y));s.pieces=s.pieces.filter(q=>q.id!==p.id);log(s,'Explosion : la pièce et la mines disparaissent.');}
